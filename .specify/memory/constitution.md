...
Modified principles: Phase III section updated to reflect AI Chatbot requirements
Added sections: Isolation Principle (new), Stateless Agent Principle (new), Model Provider Constraint (new), Tool-Driven Architecture (new)
Removed sections: Old Phase III description
...
## Governance
This constitution supersedes all other practices; Amendments require documentation, approval, and migration plan if applicable; All PRs/reviews must verify compliance with phase constraints; Complexity must be justified with clear value proposition; Each phase must be independently runnable before moving to the next phase; Code changes must follow the progressive enhancement approach from Phase I to Phase V; Phase III code must remain isolated in `/phase-3` directory; AI inputs/outputs must be sanitized; Secrets must be loaded from `.env` only.

**Version**: 1.1.0 | **Ratified**: 2026-01-02 | **Last Amended**: 2026-02-01